...
@Component({
  ...
  providers: [SucursalService, UserService]
})
...
export class ConsultorioComponent implements OnInit {
  ...
  constructor(private _sucursalService: SucursalService, location: PlatformLocation, private _route: ActivatedRoute, private _userService: UserService,
  ...
  confirmacionEliminarHorario(id_horario){
    console.log('aqui');
    this.id_horario = id_horario;
  // console.log(id_horario)
   document.getElementById('btn-confirmacion-eliminar-horario').click();

   this._sucursalService.getConfirmacionEliminarHorario(this.id_horario).subscribe( (response) => {
     console.log(response);
  ...
  eliminarHorario(){
    window.scroll(0,0);
    this.loading = true;
    this._sucursalService.dltHorarioConsultorio(this.id_horario).subscribe( (response) => {
    ...
    for(let i = 0; i < this.infoConsultorio.horario.length; i ++) {
        ...
        for(let j = 0; j < this.infoConsultorio.horario[i].dias.length; j++) {
          ...
          let nombre = this.infoConsultorio.horario[i].dias[j].dia;
            ...
            for (var k = 0; k < this.ds.length; k++) {
  ...
  guardarHorario(){
    ...
    if(this.validacionesH1() === true) {
      ...
      if (!this.mananaH1 && this.tardeH1) {
        h1 = { m_de: undefined, m_hasta: undefined, t_de: this.tardeDesdeH1 + ':00',
        t_hasta: this.tardeHastaH1 + ':00', semana : this.diasH1, id_servicio: this.infoConsultorio.id_servicios, id_sucursal : this.infoConsultorio.id_sucursales,
        id_consul : this.infoConsultorio.id_consultorio};
  ...
  guardarInformacion(){
    ...
    this.loading = true;
    let info = {nombre: this.nombreConsultorio.value, extencion : this.extensionConsultorio.value, id_consultorio : this.infoConsultorio.id_consultorio};

    this._sucursalService.editInfoConsultorio(info).subscribe( (response) => {
      // console.log(response);
      this.loading = false;
