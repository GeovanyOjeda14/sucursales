...
export class CalendarioCitasComponent implements OnInit {
          ...
          if (respuesta.length <= 0) {
            ...
            for (let i = 0; i < respuesta.length; i++) {
              ...
              let avatar = response[i].avatar;
                 ...
                 info = {id : respuesta[i].id_mascotas , tipo : 'mascota' , id_usuarios : id_usuarios , especie: especie,
                 esterilizado : esterilizado, fecha_nacimineto : fecha_nacimineto, nombre: nombre, raza: raza, sexo: sexo, avatar: avatar,
                 id_eventos: id_eventos, color : colors.prevenir};
  ...
  getEventosHistorial(mes,anio) {
    ...
    this._provedorService.getHistorialCitas(mes,anio,id_servicio,id_categoria).subscribe( (response) => {
      console.log(response);
